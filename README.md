# 👋 你好，我是 贺锦翔 | Hi, I'm He Jinxiang


### 🔬 关于我 | About Me
- 🎓 **化学专业本科生** | 具备扎实的化学理论体系与计算化学研究基础。
- 🎯 **研究志向 (Research Interests)**: 致力于探索 **AI for Science (AI4S)**，特别是**分子表示学习**与**药物发现**的交叉应用。
- 💻 **跨学科进阶**: 正在将**计算化学 (CompChem)** 的物理洞察力与**深度学习**技术相结合，探索分子的数字化表达。
- 📄 **学术进展**: 一篇关于计算化学方向的研究论文正在筹备/发表中。
- 🎯 **Ph.D. 申请者**: 2027 秋季 Ph.D. 意向申请人。

---

### 🛠️ 技术栈 | Tech Stack

#### ⚗️ 计算化学与领域背景 | CompChem & Domain Expertise
- **计算化学**: 熟悉 **DFT (密度泛函理论)** 计算、了解分子动力学模拟及相关软件。
- **理论体系**: 熟练掌握**无机、分析、有机、物化**四大化学理论，具备深厚的底层化学逻辑。并且具备**生物化学、量子化学、配位化学、催化化学**等化学细分学科知识。
- **化学信息学**:
- ![RDKit](https://img.shields.io/badge/RDKit-666666?style=flat&logo=rdkit&logoColor=white)


- **编程语言 | Languages**
  - ![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
  - `Pandas`, `NumPy`, `Matplotlib`

- **人工智能与机器学习 | AI & Machine Learning**
  - ![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white) 
  - ![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)


